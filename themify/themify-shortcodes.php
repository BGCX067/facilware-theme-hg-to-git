...
function themify_shortcodes_js_css(){
	...
	if(!themify_check('setting-google_map_script')){
		wp_enqueue_script('themify-map-script', 'http://maps.google.com/maps/api/js?sensor=false', '', THEMIFY_SHORTCODES_VERSION, true);
		wp_enqueue_script('themify-map-shortcode', THEMIFY_URI . '/js/themify.map.js', '', THEMIFY_SHORTCODES_VERSION, true);
	}
	if(!themify_check('setting-video_player_script')){
		wp_enqueue_script('themify-video-script', THEMIFY_URI.'/js/flowplayer-3.2.4.min.js', '', THEMIFY_SHORTCODES_VERSION, false);
	}
}
...
function themify_shortcode_list_posts($atts, $content = null){
	...
	if ($posts) {
		...
		foreach ($posts as $post){
			...
			if ($image == "yes") {
				if( 'no' == $unlink_image ){
					$customlistpoststr .= themify_get_image('ignore=true&w='.$image_w.'&h='.$image_h.'&alt='.get_the_title().'&before=<p class="post-image"><a href="' . $thislink . '">&after=</a></p>');
				}
				else{
					$customlistpoststr .= themify_get_image('ignore=true&w='.$image_w.'&h='.$image_h.'&alt='.get_the_title().'&before=<p class="post-image">&after=</p>');
				}
			} //$image == "yes"
