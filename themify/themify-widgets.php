...
*/
	...
	class Themify_Feature_Posts extends WP_Widget {
		...
		function form( $instance ) {
			...
			<p>
				...
				<input id="<?php echo $this->get_field_id( 'excerpt_length' ); ?>" name="<?php echo $this->get_field_name( 'excerpt_length' ); ?>" value="<?php echo $instance['excerpt_length']; ?>" size="5" /><br /><small>(leave empty = full excerpt)</small>
			</p>
	...
	*/
	...
	function themify_twitter_hyperlinks($text) {
		...
		$text = preg_replace('/([\.|\,|\:|\Â°|\Ã¸|\>|\{|\(]?)#{1}(\w*)([\.|\,|\:|\!|\?|\>|\}|\)]?)\s/i', "$1<a href=\"http://twitter.com/#search?q=$2\" class=\"twitter-link\">#$2</a>$3 ", $text);
		return $text;
	}
	...
	function themify_twitter_users($text) {
		   $text = preg_replace('/([\.|\,|\:|\Â°|\Ã¸|\>|\{|\(]?)@{1}(\w*)([\.|\,|\:|\!|\?|\>|\}|\)]?)\s/i', "$1<a href=\"http://twitter.com/$2\" class=\"twitter-user\">@$2</a>$3 ", $text);
		   return $text;
	}     
				...
				foreach ($loop as $post) {
					...
					if( $show_thumb ){
						...
						if ( has_post_thumbnail( $post->ID ) ) {
							$html .= get_the_post_thumbnail( $post->ID, array($thumb_width, $thumb_height),  array(
							...
							if( get_post_meta( $post->ID, 'post_image', true ) ) {
								$src = get_post_meta($post->ID, 'post_image', true);
								$crop = "";
								$q = ($q && isset($q) && $q != "") ? "&q=".$q : "";
								$html .= "<img src='".get_bloginfo("template_directory")."/themify/img.php?src=$src&amp;w=$thumb_width&amp;h=".$thumb_height.$crop.$q."' alt='$post_excerpt' title='$post_title' class='post-img' />";
							}
						}
					}
					...
					if ( $show_comment_count ){						
						$html .= '<small>' . get_comments_number() . ' ' . __('comments', 'themify') . '</small> <br />';
					}
					if ( $show_excerpt ) {
						$the_excerpt = get_the_excerpt();
